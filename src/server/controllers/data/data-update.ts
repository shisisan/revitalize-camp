...
import { OnDataLoad, OnDataUpdate } from './data-hooks';

@Service({ loadOrder: 1000 })
export class DataUpdateService implements OnStart {
    public constructor(private readonly database: DataService) {}
    ...
    public async onStart(): Promise<void> {
        ...
        this.database.loaded.Connect((player, data) => {
            for (const listener of dataLoadListeners) listener.onDataLoad(player, data);
            for (const listener of dataUpdateListeners) listener.onDataUpdate(player, data);
        });
